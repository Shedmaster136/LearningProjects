...
int main(int argc, char *argv[])
{
	...
	if(argc != 3)
	{
		std::cout << "Неверное количество аргументов | Wrong Arguments" << endLine;
		return -1;
	}
	int serverPort = 0;
	...
	if(serverPort == 0)
	{
		std::cout << "Неверный порт | Wrong Port Argument" << endLine;
		return -1;
	}
	...
	{
		std::cout << "Ошибка инициализации | WinSock API Initialization Failed"<< errNum << endLine;
		return -1;
	}
	...
	{
		SocketErrorHandler("Ошибка создания сокета | Socket Creation Error", errNum, serverSocket);
		return -1;
	}
	if((errNum = serverSocket.BindSocket()))
	{
		SocketErrorHandler("Ошибка привязки сокета | Socket Bind Error", errNum, serverSocket);
		return -1;
	}

	if((errNum = serverSocket.ListenSocket(2)))
	{
		SocketErrorHandler("Ошибка прослушивания подключений | Listenig Error", errNum, serverSocket);
		return -1;
	}
	...
	while(true)
	{
		if((errNum = serverSocket.AcceptConnection(clientSocket)))
		{
			...
			SocketErrorHandler("Ошибка соединения с клиентом | Client Connction Failed", errNum, serverSocket);
			return -1;
		}
		do{
